...
- **PR は小さく保つ** — 1 つの Issue/要望に対して 1 つの PR を原則とする
  ...
  - 例外: `/game-add` スキルによる新ゲーム追加（構造上 15+ ファイルになるため）
  - 超える場合は分割を提案する
...
- **Issue 作成時は以下の 2 種類のラベルを必ず付与する**:
  - **種類ラベル**（いずれか 1 つ）: `enhancement`, `bug`, `documentation`
  - **優先度ラベル**（いずれか 1 つ）: `priority: high`, `priority: medium`, `priority: low`
  - 優先度の基準:
    - `high` — 安全性リスク、日常的な開発効率への大きな影響、バグ修正（例: 危険コマンドブロック、ビルドエラー）
    - `medium` — 運用改善・利便性向上だが、現状でも支障なく動作するもの（例: スキル改善、リファクタリング）
    - `low` — nice-to-have、タイミングを選ばないもの（例: 新機能追加、テスト追加、通知機能）
