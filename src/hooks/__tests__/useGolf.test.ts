...
vi.mock("@/lib/golf-storage", () => ({
  getGolfBestScore: vi.fn(() => null),
  updateGolfBestScore: vi.fn(() => false),
}));

vi.mock(import("@/lib/golf-cards"), async (importOriginal) => {
...
describe("useGolf", () => {
  let useGolf: typeof import("@/hooks/useGolf").useGolf;
  beforeEach(async () => {
    vi.useFakeTimers();
    vi.resetModules();

    const cards = await import("@/lib/golf-cards");
    const hookModule = await import("@/hooks/useGolf");

    useGolf = hookModule.useGolf;
  ...
  describe("状態初期化", () => {
    it("初期状態でidleフェーズになる", () => {
      const { result } = renderHook(() => useGolf());
  ...
  describe("フェーズ遷移", () => {
    ...
    it("playingフェーズ以外ではremoveCardが無視される", () => {
      const { result } = renderHook(() => useGolf());
      act(() => result.current.removeCard(0));
      expect(result.current.state.phase).toBe("idle");
    });
